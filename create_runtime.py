...
def create_runtime_symlink():
    ...
    if not runtime_dir.exists():
        print(f"[status:err] '{runtime_dir}' does not exist.")
        return
    if not config_dir.exists():
        try:
            config_dir.mkdir(parents=True, exist_ok=True)
        except OSError as e:
            print(f"[status:err] Failed to create directory '{config_dir}': {e}")
            return
    ...
    try:
        ...
        print(f"[status:pass] Created runtime symlink at '{config_runtime_dir}'")
    except OSError as e:
        print(f"[status:err] Failed to create symlink: {e}")
